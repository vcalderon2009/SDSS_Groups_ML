...
"""
Main script to run the data preprocessing pipeline on the `real` data.
...
Note
-----
This pipeline is to be used on the **real** data and not synthetic
catalogues. The functions used in this script are in some fashion similar
those applied to synthetic catalogues.
"""
...
def project_const(param_dict):
    ...
    window_name     = 'SDSS_ML_data_preprocessing_data_hodn_{0}_dv_{1}'.format(
                            param_dict['hod_n'], param_dict['dv'])
    sub_window_name = 'data_preprocessing_model'
    file_exe_name   = 'catl_data_model_preprocessing_run.sh'
    ##
