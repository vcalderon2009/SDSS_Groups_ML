...
In here, we probe:
    - The difference in predictions if we train the ML algorithm on
      a given set of DV's, while keeping the HOD parameters fixed.
...
def add_to_dict(param_dict):
    ...
    dv_n_arr = num.array(param_dict['dv_models_n'].split('_')).astype(float)
...
def directory_skeleton(param_dict, proj_dict):
    ...
    out_dir = param_dict['ml_args'].catl_train_dv_diff_dir(check_exist=True,
                create_dir=True)
    #
    # Adding to `proj_dict`
...
def ml_models_training(models_dict, param_dict, proj_dict):
    ...
    ##
    ## Preparing the data
    train_test_dict = train_test_dv_diff_dict_processing(param_dict)
...
def train_test_dv_diff_dict_processing(param_dict):
    """
    Extracts the information from each of the different `DV` models, i.e.
    ...
    train_test_dv_diff_dict = {'train': {}, 'test': {}}
    # Training
    (   train_dict_main,
        test_dict_main) = param_dict['ml_args'].extract_feat_file_info()
    # Assigning as main `training` dataset
    train_test_dv_diff_dict['train']['train_dict'] = train_dict_main
    # Testing
    # Looping over the different HOD models
    for kk, dv_n_key in enumerate(param_dict['dv_n_arr']):
        # Loading parameters for this model
        model_kk = param_dict['dv_n_dict'][dv_n_key]
        ...
        train_test_dv_diff_dict['test'][dv_n_key] = test_dict_kk

    return train_test_dv_diff_dict
...
def test_dv_diff_file(param_dict, proj_dict):
    """
    ...
    ##
    ## Path to output file
    filepath = param_dict['ml_args'].catl_train_dv_diff_file(
                    check_exist=False)
    ## Saving
    ##
...
def main(args):
    ...
    (   run_opt   ,
        param_dict) = test_dv_diff_file(param_dict, proj_dict)
    # Analysis
    if run_opt:
