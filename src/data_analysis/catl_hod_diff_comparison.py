...
In here, we probe:
    - The difference in predictions if we train the ML algorithm on
      a given set of HOD's, while keeping the velocity bias parameter
      fixed.
...
def directory_skeleton(param_dict, proj_dict):
    ...
    out_dir = param_dict['ml_args'].catl_train_hod_diff_dir(check_exist=True,
                create_dir=True)
    #
    # Adding to `proj_dict`
...
def subsample_idx(train_dict, test_dict, param_dict, mass_opt='group',
    ...
    resample_opt : {'under', 'over'}, `str`, optional
        Option for which type of re-sample to use. This variable is set to
        `under` by default.
