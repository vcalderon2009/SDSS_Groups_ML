        ...
        for zz, dv_zz in enumerate(dv_arr):
            ...
            if (dv_zz == param_dict['dv']):
                ...
                ax.fill_between(x_stat,
                                ...
                                color=cm_arr[zz],
                                alpha=alpha,
                                label=r'\boldmath$f_\mathrm{vb}$: ' + '{0}'.format(dv_zz),
                                zorder=zorder_ml)
        ##
...
## Extracts the masses (HAM and DYN) for different dv models
def extract_masses_dv(dv_ii, param_dict, bin_width=0.4, arr_len=10,
    bin_statval='average', dyn_thresh=11., nlim_min=4, mass_plot='ham'):
    """
    Computes the HAM and DYN masses for different dv models

    Parameters
    ------------
    dv_ii : `int`
        dv model used for extracting masses
    ...
    Returns
    ------------
    mass_dv_ii_dict : `dict`
        Dictionary for the given `dv_ii` value, with their corresponding
        `HAM` and `DYN` masses.
    """
    ## Making copy of main `param_dict`
    param_dict_copy = param_dict.copy()
    ## Modifying dv parameters
    param_dict_copy['dv'] = dv_ii
    ## Initializing object
    ...
    mass_dv_ii_dict = {}
    mass_dv_ii_dict['dv'    ] = dv_ii
    # `HAM`
    mass_dv_ii_dict['x_stat'] = x_stat_ii
    mass_dv_ii_dict['y_stat'] = y_stat_ii
    mass_dv_ii_dict['y1'    ] = y1_ii
    mass_dv_ii_dict['y2'    ] = y2_ii

    return mass_dv_ii_dict

# Fractional Difference for each of the different dv models
def frac_diff_model_mass_dvs_panels(models_dict, param_dict, proj_dict,
    arr_len=10, bin_statval='average', fig_fmt='pdf',
    figsize_1=(8, 5), figsize_2=(15, 8), fig_number=8, nlim_min=4,
    dyn_thresh=11., mass_plot='ham'):
    """
    Plots the fractional difference between `predicted` and `true`
    halo masses.
    ...
    ## Figure name
    # fname = os.path.join(   proj_dict['figure_dir'],
    #                         'Fig_{0}_{1}_frac_diff_predicted_dv_{2}.pdf'.format(
    #                             fig_number,
    #                             param_dict['catl_str_fig'],
    #                             mass_plot))
    ##
    ## Paper Figure
    fname_paper = os.path.join( proj_dict['paper_fig_dir'],
                                'Figure_10b_{0}_panels.{1}'.format(mass_plot,
                                    fig_fmt))
    ## Algorithm names - Thought as indices for the plot
    ml_algs_names = num.sort(list(models_dict.keys()))
    n_ml_algs     = len(ml_algs_names)
    ...
    for kk, model_kk in enumerate(tqdm(ml_algs_names)):
        ...
        # Dictionary to save results
        frac_diff_dict[model_kk] = {}
        # Looping over different dv models
        for ii, dv_ii in enumerate(model_kk_data.keys()):
            # New X-axis
            model_kk_x = model_kk_data[dv_ii]['mhalo_pred']
            # Y-axis
            ...
            if (kk == 0):
                model_dv_masses_dict[dv_ii] = extract_masses_dv(
                                                    dv_ii,
                                                    param_dict,
                                                    bin_width=bin_width,
                                                    arr_len=arr_len,
    ...
    # Color
    cm       = plt.cm.get_cmap('viridis')
    cm_arr   = [cm(kk/float(n_dv)) for kk in range(n_dv)]
    propssfr = dict(boxstyle='round', facecolor='white', alpha=0.7)
    ##
    ## Constants
    xlim = (11, 15)
    ...
    for kk, ml_kk in enumerate(ml_algs_names):
        ...
        for zz, dv_zz in enumerate(dv_arr):
            ...
            ax.axhline(y=0, color='black', linestyle='--', zorder=10)
            # dv Models
            dv_str = r'\boldmath$f_\mathrm{vb}$: ' + '{0}'.format(dv_zz)
            ax.text(0.10, 0.10, dv_str, transform=ax.transAxes,
                verticalalignment='top', color='black',
                bbox=propssfr, weight='bold', fontsize=plot_dict['size_text'])
            # Plotting
            ## Extracting values for HAM
            x_stat = model_dv_masses_dict[dv_zz]['x_stat']
            y_stat = model_dv_masses_dict[dv_zz]['y_stat']
            y1     = model_dv_masses_dict[dv_zz]['y1']
            y2     = model_dv_masses_dict[dv_zz]['y2']
            ## Machine Learning Values
            x_stat_ml = frac_diff_dict[ml_kk][dv_zz]['x_stat']
            y_stat_ml = frac_diff_dict[ml_kk][dv_zz]['y_stat']
            y_err_ml   = frac_diff_dict[ml_kk][dv_zz]['y_err']
            y1_ml = y_stat_ml - y_err_ml
            y2_ml = y_stat_ml + y_err_ml
            #
            # - Plotting
            # Relation - dv Models
            ax.plot(x_stat,
            ...
            ax.fill_between(x_stat,
                            ...
                            color=cm_arr[zz],
                            alpha=alpha,
                            label='{0}'.format(mass_plot.upper()),
                            zorder=zorder_ml)
            # Relation - dv Models - ML
            ax.plot(x_stat_ml,
                    y_stat_ml,
                    color='red',
                    linestyle='--',
            ...
            # x-labels
            if (zz in range(ncols * (nrows-1), ncols*nrows)):
