...
def frac_diff_model_mass_hods_panels(models_dict, param_dict, proj_dict,
    ...
    for kk, ml_kk in enumerate(ml_algs_names):
        ...
        for zz, hod_zz in enumerate(hod_arr):
            ...
            ax.axhline(y=0, color='black', linestyle='--', zorder=10)
            # HOD Models
            ax.text(0.10, 0.10, 'HOD {0}'.format(hod_zz), transform=ax.transAxes,
                verticalalignment='top', color='black',
                bbox=propssfr, weight='bold', fontsize=plot_dict['size_text'])
            # Plotting
            ## Extracting values for HAM
            ...
            x_stat_ml = frac_diff_dict[ml_kk][hod_zz]['x_stat']
            y_stat_ml = frac_diff_dict[ml_kk][hod_zz]['y_stat']
            y_err_ml   = frac_diff_dict[ml_kk][hod_zz]['y_err']
...
def main(args):
    ...
    frac_diff_model_mass_hods(models_dict, param_dict, proj_dict,
        mass_plot='dyn')
    ## Panels
    frac_diff_model_mass_hods_panels(models_dict, param_dict, proj_dict,
        mass_plot='ham')
    frac_diff_model_mass_hods_panels(models_dict, param_dict, proj_dict,
        mass_plot='dyn')
    #
